...
//         <a href="#home" className="text-lg font-black tracking-tight">Akish<span className="text-emerald-600">.</span></a>
//         <nav className="hidden items-center gap-1 lg:flex">
//           {navLinks.map(([label, id]) => <a key={id} href={`#${id}`} className="rounded-full px-3 py-2 text-sm font-semibold text-slate-600 transition hover:bg-emerald-50hover:text-emerald-700 dark:text-slate-300 dark:hover:bg-slate-900 dark:hover:text-emerald-300">{label}</a>)}
//         </nav>
//         <div className="flex items-center gap-2">
//           <button onClick={toggleTheme} className="rounded-full border border-slate-200 p-2.5 text-slate-700 transition hover:border-emerald-400 hover:text-emerald-600 dark:border-slate-800 dark:text-slate-200" aria-label="Toggle theme">
//             {theme === 'dark' ? <Sun size={17} /> : <Moon size={17} />}
//           </button>
//           <a href="/assets/resume/Akish-Anil-Pandey-Resume.pdf" download className="hidden rounded-full bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-emerald-700 sm:inline-flex sm:items-center sm:gap-2"><Download size={16} /> Resume</a>
//           <button onClick={() => setOpen(!open)} className="rounded-full border border-slate-200 p-2.5 lg:hidden dark:border-slate-800" aria-label="Toggle navigation">{open ? <X size={18} /> : <Menu size={18} />}</button>
//         </div>
//       </div>
//       {open && <nav className="section-shell grid gap-1 border-t border-slate-200 py-4 lg:hidden dark:border-slate-800">{navLinks.map(([label, id]) => <a onClick={close} key={id} href={`#${id}`} className="rounded-xl px-3 py-2 text-sm font-semibold hover:bg-emerald-50dark:hover:bg-slate-900">{label}</a>)}</nav>}
//     </header>
//   );
// }
